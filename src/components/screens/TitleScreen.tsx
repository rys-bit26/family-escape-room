...
import { getAllRooms, getRoomById, CAMPAIGN_ROOM_ORDER } from '../../data/rooms/index.ts';
import type { DifficultyLevel, GameMode } from '../../types/game.ts';
...
export function TitleScreen() {
  ...
  const canStart = mode === 'campaign' || selectedRoomId !== null;
  ...
  function handleNewGame() {
    ...
    resetInventory();
    resetJournal();
    const roomId = mode === 'campaign' ? CAMPAIGN_ROOM_ORDER[0] : selectedRoomId!;
    startNewGame(difficulty, roomId, mode);
    navigate('/game');
  }
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center px-4 bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 overflow-y-auto py-8">
      <div className="flex flex-col items-center gap-6 max-w-lg w-full">
        ...
        {mode === 'campaign' ? (
          /* Campaign room preview */
          <div className="w-full text-center">
            <p className="text-gray-400 text-sm mb-3">Play through all four rooms in order:</p>
            <div className="flex justify-center items-center gap-1 flex-wrap">
              {CAMPAIGN_ROOM_ORDER.map((id, i) => {
                const room = getRoomById(id);
                const Icon = ROOM_ICONS[id] ?? BookOpen;
                return (
                  <div key={id} className="flex items-center gap-1">
                    {i > 0 && <ArrowRight size={12} className="text-gray-600 mx-1" />}
